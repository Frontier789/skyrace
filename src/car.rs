...
//             let turn_angle = car.max_steer_angle * car.steer;
//             let turn_cirlce_radius = car.wheel_base / turn_angle.sin();
//             let angular_velocity = car.speed() / turn_cirlce_radius;
//
//             println!("angular_velocity: {}", angular_velocity);
//             println!("angle: {}", car.angle);
//             println!("dir: {}", car.dir());
//
//             car.angle += angular_velocity * dt;
//             car.velocity = car.dir() * car.speed();
//         }
//
//         let g = 10.0;
//         let car_weight = car.mass * g;
//
//         let f_traction = car.dir() * (4500.0 * car.throttle - car.mass * car.break_ * g);
//         let f_drag = -car.c_drag * car.velocity * car.speed();
...
//
//         car.wheel_turn += car.speed() / car.wheel_radius * dt;
//
//         car_clone = Some(car.clone());
//     }
//
...
//         let car_pitch =
//             (car.front_susp.length - car.rear_susp.length).atan2(car.wheel_base);
//
//         let body_draw = world.component_mut::<DrawComponent>(*body_entity).unwrap();
//
//         body_draw.model_matrix = Mat4::offset(car.pos + Vec3::new(0.0, car_h, 0.0))
//             * Mat4::rotate_y(car.angle)
//             * Mat4::rotate_z(car_pitch)
